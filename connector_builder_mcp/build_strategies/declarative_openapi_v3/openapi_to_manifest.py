        ...
        if pagination_hint.get("type") == "offset":
            ...
            return {
                ...
                "pagination_strategy": {
                    ...
                    "cursor_value": pagination_hint.get("cursor_path", "{{ response.next_page_token }}"),
                    "stop_condition": pagination_hint.get("stop_condition", "{{ not response.next_page_token }}"),
                },
                "page_token_option": {
                    "type": "RequestOption",
...
def generate_manifest_from_openapi(
    ...
    if not streams:
        warnings.append("No candidate streams found. Only GET operations returning lists are supported in MVP.")
