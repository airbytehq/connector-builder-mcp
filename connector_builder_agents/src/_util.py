...
def get_secrets_dotenv(
    ...
    if existing_connector_name and existing_config_name:
        ...
        connector_short_name = "_".join(existing_connector_name.split("-")[1:])
        secrets_lines.extend(
            f"{connector_short_name.upper()}_{key.upper()}='{value}'"
            for key, value in secrets_dict["credentials"].items()
        )
